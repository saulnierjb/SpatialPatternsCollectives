...
from parameters_igoshin_rp import Parameters
...
class Simulation1D:
    ...
    def __init__(self, initial_density, signal_threshold, fluctuation_level):
        ...
        self.U = 0.5 * (self.initial_density * self.par.w_0 * (self.par.w_0 + self.par.w_n)
             / (self.par.w_n * self.phi_r(initial_density) + cp.pi * self.par.w_0))
        self.u0 = self.U / (self.par.w_0 + self.par.w_n)
        ...
        # Save
        self.path_folder_sample = 'results/sample_igoshin_rp__q=' + str(round(self.par.q, 1)) +'__phi_r_w_n_sur_w_0=' + str(round(self.phi_r(self.initial_density) * self.par.w_n / self.par.w_0, 2)) + '__initial_density=' + str(round(self.initial_density, 2)) + '__signal_threshold=' + str(round(self.signal_threshold, 2)) + '/'
        self.tool.initialize_directory_or_file(self.path_folder_sample)
        self.path_file_kymograph = self.path_folder_sample+'kymograph/data_kymo__time_init='+str(self.par.start_time_save_kymo)+'save_frequency_kymo='+str(self.par.save_frequency_kymo)+'_minutes'+'_igoshin_rp.csv'
    ...
    def phi_r(self, rho):
        ...
        """
        return self.par.phi_mid * self.signal_threshold**self.par.q / (self.signal_threshold**self.par.q + rho**self.par.q)
    ...
    #     """
    #     return self.par.delta_phi_r * self.signal_threshold / rho
    

    def w(self, rho):
        """
        Function \omega^\pm(rho)
        
        """
        # The Heaviside function
        self.sigmoid = 1 / (1 + cp.exp(-self.par.alpha_sigmoid * (self.par.rrep - self.phi_r(rho))))

        return self.par.w_0 + self.par.w_n* self.sigmoid
    ...
    def plot_kymograph(self, cmap, vmin=None, vmax=None):
        ...
        fig.savefig(self.path_folder_sample+'kymograph/kymo_igoshin_rp.png', bbox_inches='tight', dpi=self.par.dpi)
        fig.savefig(self.path_folder_sample+'kymograph/kymo_igoshin_rp.svg', dpi=self.par.dpi)
...
print('w_n/w_0 =', sim.par.w_n / sim.par.w_0)
